...
## [Sat Feb 28 16:30:00 EST 2026] Phase 6 - CLI Entry Point and Streaming Checksums
- Created `cmd/gfast/main.go` as the primary CLI entry point with full argument parsing.
- Implemented streaming CRC64 checksum support in `engine/checksum.go` with `ChecksumWriter` and `ChecksumReader`.
- Added `ChecksumPool` for reusable hash instances to minimize allocations during high-throughput transfers.
- Integrated TUI with real-time worker scaling via signal handlers (SIGUSR1/SIGUSR2).
- Added comprehensive integration tests in `provider/integration_test.go` covering:
  - Local-to-local file transfers
  - Metadata preservation across transfers
  - Nested directory creation
  - Concurrent transfer operations
- Updated `README.md` with installation instructions, CLI options, and usage examples.
- *Decisions Made*:
  - CRC64 chosen over XXHash for streaming due to hardware acceleration on modern CPUs and sufficient collision resistance for integrity verification.
  - Checksum validation is optional (via `-checksum` flag) to avoid performance overhead when not needed.
  - TUI enabled by default but can be disabled for headless/daemon operation.
  - State directory defaults to `./.gofast-state` for easy cleanup and portability.
